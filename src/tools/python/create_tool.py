#!/usr/bin/env python3
"""
Shared tool: create_tool
Creates a new Python execution tool in the tenant's execution folder.
...
It will NOT overwrite existing tools - use update_tool.py for that.
"""
...
def tool_exists(name: str) -> bool:
    """Check if tool already exists in execution folder."""
...
def update_manifest(name: str, description: str, input_schema: dict) -> bool:
    """Add new tool entry to tool_manifest.json."""
...
def main():
    ...
    input_schema = input_data.get("input_schema", {"type": "object", "properties": {}})
    ...
    if tool_exists(name):
        print(json.dumps({
            "success": False,
            "message": None,
            "path": os.path.join("execution", f"{name}.py"),
            "manifest_updated": False,
            "test_result": None,
            "error": f"Tool '{name}' already exists. Use update_tool to modify it."
        }))
        sys.exit(1)
    ...
    # Move from staging to execution
    final_path = os.path.join("execution", f"{name}.py")
    try:
        ...
        print(json.dumps({
            ...
            "manifest_updated": False,
            "test_result": test_result,
            "error": f"Failed to deploy tool: {str(e)}"
        }))
        sys.exit(1)
    ...
    try:
        ...
        # Tool was created but manifest update failed - warn but don't fail
        print(json.dumps({
            "success": True,
            "message": f"Tool '{name}' created but manifest update failed: {str(e)}",
            "path": final_path,
            "manifest_updated": False,
            "test_result": test_result,
            "error": None
